...
- `grantKit` stores the kit in the backpack and sets the character's `MiscData["archetype-kit"]` marker, then saves the user. The items and the marker live in the same user file, so they are always persisted together.
