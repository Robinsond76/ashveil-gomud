...
- A band is the present members in one room (two or more). Its tier comes from the average of their **saved** service (`Service.Saved`, in memory only, `yaml:"-"`): `save()` and `load()` call `markServiceSaved`, so tiers never use rounds that aren't on disk. When a round would lift a band's tier, accrual saves at once and tells the leader after; a failed save just retries next round. Keep every company save going through `m.save()`, or service will lag behind what's on disk.
