...
- **Expiry** at zero goes through `dropCompanion` (the dismissal path with rollback) and adds the companion to `Record.Lost` (ten most recent) in the same save. `Put` keeps a record that has only `Lost`.
