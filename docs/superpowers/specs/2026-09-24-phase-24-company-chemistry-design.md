...
4. **The band and dilution.** A member's band is every present member in
   ...
   average service sets its tier, and **everyone in the band** gets that
   tier's bonus, a new recruit included. In the average, a member with no
   service counts as 0, and each member counts for **at most the Sworn
   threshold**, so long service can't hide a recruit: four veterans and a
   fresh recruit average at most 5040 (Trusted), and a veteran leader and a
   fresh recruit at most 3150 (Trusted). Integer division. Only one tier
   applies; nothing stacks. Stored service is not capped, so the display
   shows real time served.
5. **Durability.** Each entry also keeps `Saved`, in memory only: its
   ...
   on disk, whoever walks in or out of the room. Chemistry adds **no save
   of its own**: service rides on the existing save seams (autosave every
   225 rounds, drift ticks every 75, logout, copyover, shutdown, commands),
   so the company file (which also holds gear snapshots) still changes only
   where it did before. A band whose current service would lift its tier
   is noted; after the next successful save the tier takes effect and the
   leader is told once, at the highest tier reached (a band that no longer
   reaches it, say after a dismissal, is dropped silently). A failed save
   leaves the tier where it was. A crash can lose the rounds since the last
   save.
...
9. **Surfaces.** `company chemistry` shows the band with the leader (size,
   ...
   named); leader first, then companions by number. The browser Company panel is deferred to the
...
- `modules/company/chemistry.go`: the `chemistryWorld` seam, cached config,
  per-round accrual from `onNewRound`, pending tier-ups, the provider,
  `company chemistry`; `save` marks service saved and announces tier-ups,
  `load` marks loaded service saved.
...
- Module: service accrues only for members present with another
  ...
  service; a respawned companion resumes; a tier-up takes effect and is
  announced at the next save, once; a failed save keeps the tier until
  saved and announces nothing; a veteran's service is capped in the
  average; a
