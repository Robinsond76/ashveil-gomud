...
- [ ] Tests first (`internal/mobcommands`): a practice mob's `Suicide`
  ...
  rewards.
- [ ] `mobs.Mob.Practice` (`yaml:"practice"`); `mobcommands.PracticeBeaten`,
...
- [ ] Tests first (`stages_test.go`, `tutorial_test.go`): order and rooms
  ...
  the view.
- [ ] `StageCombat`; seams `spawnFoe`, `removeFoe`; `foes` in memory;
  `onPracticeBeaten`; config `PracticeSquad`.

## Task 3: Content

- [ ] Tests first (`shipped_test.go`): seven rooms; the Practice Yard;
  mobs 67 and 68 practice, `dummy` race, `practice-squad`; help mentions
  Combat.
- [ ] Room 906; 903's back exit to 906; map; `TutorialRooms`; mobs 67
  and 68; help template; config overlay.

## Task 4: Wiring test

- [ ] Through `plugins.Load` and the real combat round: the party
...
- [ ] `modules/tutorial/AGENTS.md`; `internal/mobcommands` note.
