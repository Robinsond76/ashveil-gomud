...
- [x] Tests first (`modules/encumbrance/shipped_weights_test.go`): every
...
- [x] Set `weight` in every item YAML.
...
- [x] Tests first:
  ...
  - `modules/encumbrance`: the load adds it, and the `cargo` status shows
    it.
- [x] `Load.CompanionGrams`; `company.GearProvider`/`CompanionGearGrams`;
  `CompanyModule.CompanionGearGrams`; the encumbrance module reads it.

## Task 3: Wiring

- [x] Through `plugins.Load` with the company and encumbrance modules
...
- [x] `modules/encumbrance`, `modules/company` guides; refresh the stale
  "Known issues" list in `docs/PROJECT_STATUS.md`.
- [x] `go test -race ./...`, `make generate`, `make validate`.
- [x] Independent review; verify findings; record with a **Review:** line.
