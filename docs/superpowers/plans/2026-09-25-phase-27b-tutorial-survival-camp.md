...
- [ ] Tests first: `internal/survival` — `OnProvision` fires on a
...
- [ ] `survival.Provisioned` and `survival.OnProvision` (fired in the
  `Provision` wrapper).
- [ ] `usercommands.IsRegistered(cmd)`.
- [ ] `camping.CampAbandoner` / `SetCampAbandoner` / `AbandonCamp`.
...
- [ ] Tests first (`modules/camping/abandon_test.go`): an idle camp, a
...
- [ ] `AbandonCamp`, sharing the camp half of `AbandonForDeath`; registered
  in `init`.

## Task 3: Stages as data

- [ ] Tests first (`stages_test.go`): order Character, Company, Formation,
...
- [ ] `Stage.Inspections`; Survival and Camp stages; `required`;
  `inspected`; `survivalDone`.

## Task 4: Module behaviour

- [ ] Tests first (`tutorial_test.go`): inspections count in their own
...
- [ ] `modules/tutorial/AGENTS.md`, `modules/camping` notes.
