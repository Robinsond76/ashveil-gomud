...
- [ ] **`internal/market`: `AskForStock`, `BidForStock`.**
  ...
  - Then implement.
- [ ] **`modules/market`: market room tag, two-price listing.**
  ...
  - Then implement.
- [ ] **`modules/market`: `market buy` / `market sell`.**
...
- [ ] **Content:** rooms 2004 (Dunmar Market Square) and 2005 (Trappers'
      Post), tagged `market`; West Gate east exit and description line;
      Fork east exit.
- [ ] **Wiring test** (extend `TestMarketEndToEndThroughPluginsLoad`):
...
- [ ] Update `docs/PROJECT_STATUS.md` (Phase 19b entry with **Review:**
