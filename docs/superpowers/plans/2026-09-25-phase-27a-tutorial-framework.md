...
- [x] **Task 1: seams.** `internal/tutorial` (`Provider`, `Begin`, `Active`),
...
- [x] **Task 2: stages and progress** (`modules/tutorial/stages.go`). Tests
  first: `TestStageOrder`, `TestProgressRoundTrip`, `TestCharacterGate`,
  `TestCompanyGate`, `TestFormationGate`.
- [x] **Task 3: the course** (`modules/tutorial/tutorial.go`): `Begin`
...
- [x] **Task 4: the `tutorial` command.** Tests first: status, next, and
  skip with its confirmation.
- [x] **Task 5: content.** Rooms 900–903 rewritten, JS removed, the Muster
