...
riders and applies to walking only. Implemented 2026-09-23; every task is
done. The design doc's "Implementation notes" lists the deviations.
...
- [x] **1. `internal/walking` (pure).** Files: `internal/walking/walking.go`,
  ...
  - Then implement.
- [x] **2. Read seams.** Files: `internal/encumbrance/provider.go`,
  ...
  - Data: pack-horse `FatiguePct: 75`, `Riders: 2`; config comments no longer say
    "informational".
- [x] **3. Travel multipliers (`internal/expedition` + `modules/expedition`).**
  - Tests first, domain:
...
- [x] **4. Camp rest multiplier (`internal/camping` + `modules/camping`).**
  - Tests first:
...
- [x] **5. Inns (`internal/camping` + `modules/camping`).** New file
...
- [x] **6. `modules/walking`.** Files: `modules/walking/walking.go`,
...
- [x] **9. Concurrency test.** Under `-race`, run concurrently: `Stepped`
...
- [x] **13. Record.** Add a Phase 16 entry to `docs/PROJECT_STATUS.md` (with
