...
- [x] **Wiring: `internal/mobcommands/suicide.go`'s death-drop block.**
  - Test first (real command path, not just the pure package): a mob
    ...
    same corpse-vs-floor branch the existing worn-item drops use. Exercise
    both corpse and floor configurations: inspect corpse/room contents
    and assert `MobItemDrop` is emitted for floor drops only, matching
    `suicide.go`'s current behavior.
  - Regression tests: a mob with `LootCategory == ""` drops identically
    to today; an entry whose item spec is unavailable at death produces
    no zero item and does not disturb worn-item or gold drops.
  - Then implement: after the existing worn-item loop
    ...
    `RollCount`, check `items.GetItemSpec(entry.ItemID)` before creating
...
- [x] **Content: category tag + tables for a proving slice.**
  ...
  - Author `_datafiles/world/<world>/loot/humanoid.yaml` and `beast.yaml`
    ...
    trade-good entry (e.g. a wolf hide/fur), an edible raw ingredient and
...
- [ ] **Recipe requirements and deterministic selection.** Add an optional
      ...
      ready-recipe selection deterministic by ascending output item ID.
      In `usercommands.Use`, select the first ready recipe the actor can
      make, check its skill before consuming inputs, and refuse when only
      gated recipes are ready.
  - Tests first through `use <container>`: skilled cooking consumes
