...
const snapshot = {
  leader: { key: 'leader', id: 0, name: 'Wren', status: 'present', level: 5, archetype: 'Ranger', cell: { row: 0, col: 0 }, rescue_seconds: null, chemistry: 'Trusted' },
  members: [
    { key: 'companion:1', id: 1, name: 'Bran', status: 'present', level: 3, archetype: 'Warrior', cell: { row: 0, col: 1 }, rescue_seconds: null, chemistry: 'Trusted' },
    { key: 'companion:2', id: 2, name: '<img src=x onerror="window.__xss=1">', status: 'awaiting', level: 2, archetype: null, cell: null, rescue_seconds: null, chemistry: null },
    { key: 'companion:3', id: 3, name: 'Ysolde', status: 'dead', level: 5, archetype: 'Cleric', cell: null, rescue_seconds: 5400, chemistry: null },
  ],
  alive: 3, dead: 1,
...
// Logged in: one request for the company, however many Char payloads come.
await page.evaluate(() => { window.gmcp('Char.Vitals', {}); window.gmcp('Char.Vitals', {}); });
check(JSON.stringify(await page.evaluate(() => window.requests)) === '["Company"]', 'asks for the company once after login');
...
// A vitals-only update merges and keeps the roster.
await page.evaluate(() => window.gmcp('Company.Vitals', { vitals: { 'companion:1': { hp: 5, hp_max: 25, needs: null, warmth: null } } }));
const t2 = await text();
...
// Keyboard: the first card takes focus.
